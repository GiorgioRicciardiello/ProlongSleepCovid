"""
Easily create a Table One fro a DataFrame
...
class MakeTableOne:
    def __init__(self, df: pd.DataFrame,
                 continuous_var: list[str] = None,
                 categorical_var: list[str] = None,
        ...
        usage:
            ...
            df_table_one_clean = table_one_constructor.group_variables_table()
        ...
        """
        self.df = df
        self.continuous_var = continuous_var or []
        self.categorical_var = categorical_var or []

        # Raise an error if both continuous_var and categorical_var are None or empty
        if not self.continuous_var and not self.categorical_var:
            raise ValueError("Both continuous_var and categorical_var cannot be None or empty. "
                             "At least one must be provided.")

        self.strata = 'SingleDistributionTable' if strata is None else strata

        # Combine the variables into the index
        self.index = self.continuous_var + self.categorical_var
    ...
    def _create_index(self, frame: pd.DataFrame,
        ...
        if all([isinstance(val, str) for val in unique_vals]):
            ...
            raise ValueError(f'The column {col} is of mixed data types, strings and numeric. Please set to one or the '
                             f'other')
        # Check if it's sparse and convert if necessary
        if isinstance(unique_vals, pd.SparseDtype):
            unique_vals = unique_vals.to_dense()
    ...
    def _continuous_var_dist(frame: pd.DataFrame,
        ...
        cell = f'{np.round(frame[col].dropna().mean(), decimal)} ({np.round(frame[col].dropna().std(), decimal)})'
        return cell
